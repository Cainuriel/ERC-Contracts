...
contract TRUSTY is Context, IERC20, Ownable {
	...
	string private _name = "In us we ";
	string private _symbol = "TRUSTY";
	...
	uint256 public _sametaxFee = _charityfee + _burnFee + _marketingFee + _rewardFee;
	...
	private view returns (uint256, uint256, uint256, uint256, uint256, uint256) 
	{
		...
		=  _getRValues(tAmount, tFee, tProject,  _getRate());
	...
	function _getTValues(uint256 tAmount) private view returns (
		...
		uint256 tFee = calculateRewardFee(tAmount); // se usara con las cuatro fees de 1%
		uint256 tProject = _getTaxProject(tAmount);
		uint256 tTotalSubstration = tFee.mul(_sametaxFee);
		uint256 tTransferAmount = tAmount.sub(tProject).sub(tTotalSubstration);

        

		return (tTransferAmount, tFee, tProject);
	}
  ...
  private view returns (uint256, uint256, uint256) {
    ...
    uint256 rtotalSubstration = rFee.mul(_sametaxFee);
    uint256 rTransferAmount = rAmount.sub(rtotalSubstration).sub(rProject);
...
//     return (charityFee, burnFee, marketingFee, projectFee);
//   }
	...
	function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
		(
			...
			uint256 tTransferAmount,
			uint256 tFee,
			uint256 tProject) = _getValues(tAmount);
			
	    _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);   
    
		_transferBurn(sender, tFee);
        _transferProject(sender, tProject);
        _transferCharity(sender, tFee);
        _transferMarketing(sender, tFee);
		_HODLrFee(rFee, tFee);
