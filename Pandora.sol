...
contract Pandora is Context, IERC20, Ownable, ReentrancyGuard {
	...
	{
		(uint256 tTransferAmount, uint256 tHODLrFee, uint256 tBurn, uint256 tProject)
		= _getTValues(tAmount);
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee)
		= _getRValues(tAmount, tHODLrFee, tBurn, tProject, _getRate());
		return (rAmount, rTransferAmount, rHODLrFee, tTransferAmount, tHODLrFee, tBurn, tProject);
	}
	...
	function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
		...
		return (tTransferAmount, tHODLrFee, tBurn, tProject);
	}

	function _getRValues(uint256 tAmount, uint256 tHODLrFee, uint256 tBurn, uint256 tProject, uint256 currentRate) 
	...
	function _transferBurn(uint256 tBurn) private {
		...
		function _transferProject(uint256 tProject) private {
		uint256 currentRate = _getRate();
		uint256 rProject = tProject.mul(currentRate);		
